...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { email: user.email, tenantId: user.tenantId, role: user.role },
...
router.put("/upgrade", async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ email }, { plan: plan || "Pro" });
    res.json({ message: `Upgraded to ${plan || "Pro"}` });
  } catch (error) {
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/team/:tenantId", async (req, res) => {
  try {
    const members = await User.find({ tenantId: req.params.tenantId }).select("-password");
    res.json(members);
  } catch (err) {
...
router.get("/activity/:tenantId", async (req, res) => {
  try {
    const activities = await Activity.find({ tenantId: req.params.tenantId }).sort({ createdAt: -1 });
    res.json(activities);
  } catch (err) {
...
router.put("/billing-cycle", async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ email }, { billingCycle });
